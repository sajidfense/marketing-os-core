...
export async function getOrgSettings(req: Request, res: Response): Promise<void> {
  const [orgResult, brandingResult] = await Promise.all([
    supabase.from('organizations').select('*').eq('id', req.organizationId).single(),
    supabase.from('branding_settings').select('*').eq('organization_id', req.organizationId).single(),
  ]);
...
export async function updateBranding(req: Request, res: Response): Promise<void> {
  ...
  const { data, error } = await supabase
    ...
    .upsert({ ...body, organization_id: req.organizationId }, { onConflict: 'organization_id' })
    .select().single();
...
export async function getUsage(req: Request, res: Response): Promise<void> {
  ...
  const data = await getOrgUsageSummary(req.organizationId!, days);
  res.json({ success: true, data });
}
...
export async function inviteMember(req: Request, res: Response): Promise<void> {
  ...
  const { data, error } = await supabase
    ...
    .upsert({ organization_id: req.organizationId, user_id: user.id, role }, { onConflict: 'organization_id,user_id' })
    .select().single();
  if (error) throw error;
